...
fn load_stations(path: &str) -> Vec<(Vec<u8>, f64)> {
    ...
    for line in raw.lines() {
        ...
        let Some((name, mean)) = line.split_once(';') else { continue };
        let Ok(mean) = mean.trim().parse::<f64>() else { continue };
...
fn main() {
    ...
    let out = args.get(2).map(|s| s.as_str()).unwrap_or("measurements.txt");
    let csv = args
        .get(3)
        .map(|s| s.as_str())
